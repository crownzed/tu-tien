...
class GameService {
  ...
  playerAttack() {
    ...
    // Công thức STR-based ATK mới — nhân β cảnh giới để khớp HP/quái (cùng scale theo β).
    const beta = StatsEngine.calcRealmMultiplier(realmTier);
    const baseAtk = StatsEngine.calcRawATK(eqBonuses.attackBonus, attrs.str) * beta;
  ...
  playerCast() {
    ...
    // Công thức INT-based MATK mới (Thi Pháp dùng INT thay vì STR) — nhân β khớp HP/quái.
    const beta = StatsEngine.calcRealmMultiplier(realmTier);
    const baseMAtk = StatsEngine.calcRawMATK(eqBonuses.attackBonus, attrs.int) * beta;
  ...
  castCongPhapSkill(skillId) {
    ...
    const beta = StatsEngine.calcRealmMultiplier(realmTier);
    const skillAtk = skill.damage * evolvedScale * beta;
    const playerDmg = CombatEngine.calculateDamage(
  ...
  _rollManualDrop(combat) {
    ...
    // Gate theo cảnh giới: rớt rarity cao hơn cảnh giới hiện tại → hạ về rarity cao nhất
    // mà cảnh giới cho phép (tránh kiếp đầu vơ công pháp huyền thoại).
    const run = this.repos.run.get();
    const realmOrder = run ? (this.data.realmById[run.realm_id]?.order || 1) : 1;
    const rarityOrder = ['common', 'rare', 'epic', 'legendary', 'mythic'];
    while (rarity && realmOrder < this._rarityMinRealm(rarity)) {
      const idx = rarityOrder.indexOf(rarity);
      if (idx <= 0) { rarity = null; break; }
  ...
  _advanceRealm(run, btResult) {
    ...
    this.repos.run.update({
      realm_id: run.realm_id, realm_stage: run.realm_stage, hp: run.hp, hp_max: run.hp_max,
      linh_khi: run.linh_khi, linh_khi_max: run.linh_khi_max,
      tu_vi: run.tu_vi,
  ...
  travelStep() {
    ...
    // Lọc shop thương nhân theo cảnh giới: ẩn món rarity cao chưa đủ cảnh giới để mua
    // (tránh UI hiện món rồi báo lỗi khi bấm). Clone scenario để không làm bẩn data cache.
    if (step.scenario?.shop && Array.isArray(step.scenario.shop)) {
      const realmOrder = this.data.realmById[run.realm_id]?.order || 1;
      const allowed = step.scenario.shop.filter(s => {
        const { baseId } = Items.parseItemId(s.item);
        return realmOrder >= this._rarityMinRealm(this.data.itemDefs[baseId]?.rarity);
      });
      step.scenario = { ...step.scenario, shop: allowed };
    }
  ...
  travelBuy(shopItemId, cost) {
    ...
    // Gate theo cảnh giới: đồ rarity cao chỉ mua được khi đạt cảnh giới tương ứng,
    // chặn người chơi cảnh giới thấp vơ đồ huyền thoại/thần thoại quá sớm.
    const realmOrder = this.data.realmById[run.realm_id]?.order || 1;
    const { baseId } = Items.parseItemId(shopItemId);
    const itemDef = this.data.itemDefs[baseId];
    const minRealm = this._rarityMinRealm(itemDef?.rarity);
    if (realmOrder < minRealm) {
      return { ok: false, error: `Cần cảnh giới ${minRealm} (${itemDef?.rarity || '?'}) để mua ${itemDef?.name || shopItemId}.` };
    }
  ...
  /** Cảnh giới tối thiểu (realm order) để nhận/mua item theo rarity. */
  _rarityMinRealm(rarity) {
    switch (rarity) {
      case 'mythic':    return 9;
      case 'legendary': return 7;
      case 'epic':      return 5;
      case 'rare':      return 3;
      default:          return 1;  // common / không rõ
    }
  }
