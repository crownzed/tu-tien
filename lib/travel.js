...
/**
 * Chọn scenario từ pool, có lọc theo flags.
 */
function pickScenario(scenarios, type, flags = {}, rng = Math.random) {
  const pool = scenarios[type];
  if (!pool || pool.length === 0) return null;

  // Lọc theo flag conditions
  const eligible = pool.filter(s => checkTravelConditions(s, flags));
...
/**
 * Thực hiện 1 bước travel. Hỗ trợ flags.
 */
function travelStep(stepTypes, scenarios, luck = 0, flags = {}, rng = Math.random) {
  const stepType = rollStepType(stepTypes, luck, rng);
  const scenario = pickScenario(scenarios, stepType.type, flags, rng);
  ...
  if (scenario.reward) {
    ...
    if (r.spiritStones) {
      if (Array.isArray(r.spiritStones)) {
        ...
        run.spirit_stones += r.spiritStones;
        result.reward = { spiritStones: r.spiritStones };
        result.message += ` Nhận ${r.spiritStones} Linh Thạch.`;
      }
    }
    if (r.item) {
