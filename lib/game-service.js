...
class GameService {
  ...
  enterNode() {
    ...
    // Treasure / Rest / NPC / Event -> dùng event scripts
    const script = EventChain.pickEventScript(this.data.eventScripts, node.type, this.rng);
    if (!script) {
      // Fallback: xử lý cơ bản cho node không có script
      return this._handleBasicNode(run, mapState, node);
    }

    // Chuyển sang IN_EVENT — khóa di chuyển, chỉ cho chọn lựa
    const fsmEvent = new FSM(run.fsm_state);
    const trans = fsmEvent.transition(STATES.IN_EVENT);
    if (!trans.ok) return { ok: false, error: trans.error };

    // Lưu script đang active vào map state
    ...
    return { ok: true, nodeType: node.type, node, action: 'event', script };
  }
  ...
  resolveChoice(choiceIndex) {
    ...
    // Validate: CHỈ được chọn khi đang IN_EVENT
    ...
    const result = EventChain.resolveChoice(script, choiceIndex, run.luck, this.rng);
    if (!result.ok) return result;

    const res = result.result;

    // Áp dụng reward/damage/cost
    if (res.reward) EventChain.applyReward(run, res.reward);
    if (res.damage) EventChain.applyDamage(run, res.damage);
    if (res.cost || (res.reward && res.reward._item)) {
      EventChain.applyCost(run, res.cost || null);
    }

    // Xử lý item thưởng
    const items = [];
    if (res.reward && res.reward._item) items.push(res.reward._item);
    if (res.reward && res.reward._item2) items.push(res.reward._item2);
  ...
  _countArtifacts() {
    ...
    const artifactIds = ['phap_bao_so_cap', 'phap_bao_trung_cap', 'ky_bao_ha_pham', 'pha_gioi_dan'];
  ...
  travelStep() {
    ...
    const step = Travel.travelStep(travelData.stepTypes, travelData.scenarios, run.luck, this.rng);
