...
function attemptBreakthrough(realm, run, linhCan, artifactCount = 0, rng = Math.random) {
  ...
  if (realm.breakthrough.hasTribulation) {
    ...
    // §5 Sát thương sét theo %HP + ωWrath (thay damage cố định).
    //   artifact giảm sát thương → đóng vai mitigation [0, 0.9].
    const mitigation = Math.min(0.9, artifactCount * balance.breakthrough.artifactTribulationReduction);
    const effectiveDmg = StatsEngine.calcTribulationDamage(run.hp_max, run.luck, mitigation);
